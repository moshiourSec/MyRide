...
class HomeVC: UIViewController, Alertable {
        ...
        UpdateService.instance.observeTrips { (tripDict) in
            if let tripDict = tripDict {
                ...
                if acceptanceStatus == false {
                    DataService.instance.isDriverAvailable(key: (Auth.auth().currentUser?.uid)!, handler: { (available) in
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        DataService.instance.isDriverAvailable(key: (Auth.auth().currentUser?.uid)!, handler: { (status) in
            if status == false {
